...
class Mask2FormerHead(MaskFormerHead):
    ...
    def forward(self, feats, img_metas):
        ...
        query_embed = self.query_embed.weight.unsqueeze(1).repeat(
            (1, batch_size, 1))
        # print(self.query_feat.weight.shape, self.query_embed.weight.shape,query_feat.shape,query_embed.shape)
        cls_pred_list = []
        mask_pred_list = []
        cls_pred, mask_pred, attn_mask = self.forward_head(
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:])
        # print(cls_pred.shape, mask_pred.shape, attn_mask.shape)
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            cls_pred_list.append(cls_pred)
            mask_pred_list.append(mask_pred)
        # print(len(cls_pred_list), len(mask_pred_list))
